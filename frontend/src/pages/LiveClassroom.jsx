...
const startRecording = async () => {
    try {
        ...
        alert("Could not start recording. Please grant permissions.");
    }
};
                ...
                <div style={{ height: '70px', display: 'flex', alignItems: 'center', gap: '15px', padding: '0 10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                        {['#4F46E5', '#EF4444', '#10B981', '#000000'].map(c => (
                            <div key={c} onClick={() => changeColor(c)} style={{ width: '25px', height: '25px', borderRadius: '50%', background: c, border: color === c ? '3px solid white' : '1px solid #ddd', cursor: 'pointer' }} />
                        ))}
                    </div>
                    ...
                    <button onClick={isRecording ? stopRecording : startRecording} style={{ background: isRecording ? '#EF4444' : '#10B981', color: 'white', border: 'none', padding: '8px 15px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}>
                    ...
                    <button onClick={endClass} style={{ background: '#EF4444', color: 'white', border: 'none', padding: '8px 12px', borderRadius: '5px', cursor: 'pointer' }}>End</button>
                </div>
            </div>
        )}

        {/* Right: Jitsi Call Area */}
        <div style={{ background: '#1E293B', height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
